...
pub type EonInt = i64;
pub type EonFloat = f64;
#[cfg(feature = "ffi")]
use crate::addons::EON_VM_FUNCS;
#[cfg(feature = "ffi")]
use crate::addons::EonVmFunctions;
...
pub struct Vm<Value: ValueTrait = PackedValue> {
    ...
    libs: Vec<Library>,
    #[cfg(feature = "ffi")]
    foreign_functions: Vec<unsafe extern "C" fn(*mut c_void, *const EonVmFunctions) -> ()>,
}
pub enum VmStatus {
    Done,
    PendingHostFunc(u16),
...
impl<Value: ValueTrait> Vm<Value> {
    ...
    #[inline(always)]
    pub fn push_int(&mut self, n: EonInt) {
        self.push(n);
    }
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn push_float(&mut self, f: EonFloat) {
        self.push(f);
    }
    #[inline(always)]
...
{
    fn make_nil() -> Self;
    fn get_int(&self, vm: &Vm<Self>) -> EonInt
    where
        Self: Sized;
    fn get_float(&self, vm: &Vm<Self>) -> EonFloat
    where
        Self: Sized;
...
pub struct PackedValue(u64,  bool);
#[derive(Debug, Clone, Copy)]
...
impl From<EonInt> for TaggedValue {
    #[inline(always)]
    fn from(n: EonInt) -> Self {
        Self::Int(n)
    }
}
impl From<EonFloat> for TaggedValue {
    #[inline(always)]
    fn from(n: EonFloat) -> Self {
        Self::Float(n)
    }
}
...
impl From<EonInt> for PackedValue {
    #[inline(always)]
    fn from(n: EonInt) -> Self {
        Self(n as u64, false)
    }
}
impl From<EonFloat> for PackedValue {
    #[inline(always)]
    fn from(n: EonFloat) -> Self {
        Self(EonFloat::to_bits(n), false)
    }
}
...
impl ValueTrait for PackedValue {
    ...
    #[inline(always)]
    fn get_int(&self, _vm: &Vm<Self>) -> EonInt {
        self.0 as EonInt
    }
    #[inline(always)]
    fn get_float(&self, _vm: &Vm<Self>) -> EonFloat {
        EonFloat::from_bits(self.0)
    }
    #[inline(always)]
...
impl<Value: ValueTrait> Vm<Value> {
    ...
    fn step(&mut self) -> bool {
        ...
        match instr {
            ...
            Instr::DivideInt => {
                ...
                let a = self.pop_int();
                if b == 0 {
                    self.error = Some(Box::new(self.make_error(VmErrorKind::DivisionByZero)));
                    return false;
                }
            ...
            Instr::CallExtern(_func_id) => {
                ...
                #[cfg(feature = "ffi")]
                {
                    unsafe {
                        ...
                        let eon_vm_functions_ptr = &EON_VM_FUNCS as *const EonVmFunctions;
                        self.foreign_functions[_func_id as usize](vm_ptr, eon_vm_functions_ptr);
                    };
                }
            }
        }
    ...
    #[inline(always)]
    pub fn pop_int(&mut self) -> EonInt {
        self.pop().get_int(self)
    }
    #[inline(always)]
    pub fn pop_float(&mut self) -> EonFloat {
...
impl Display for VmError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        let width = std::iter::once(&self.location)
            ...
            .map(|loc| loc.filename.len() +  1 + loc.lineno.to_string().len())
            .max()
            .unwrap_or(10);
        writeln!(f, "[traceback]")?;

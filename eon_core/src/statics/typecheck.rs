...
pub(crate) enum Prov {
    ...
    InstantiateInterfaceOutputType(Rc<InterfaceImpl>, Rc<InterfaceOutputType>),
    FuncArg(AstNode, u8),
    FuncOut(AstNode),
    ListElem(AstNode),
}
...
fn constrain_locked_typevars(
    ...
    if key1 != key2 {
        ...
        match (potential_ty1, potential_ty2) {
            ...
            (PotentialType::Nominal(_, _, type_vars1), PotentialType::Nominal(_, _, type_vars2)) => {
                for (tv1, tv2) in type_vars1.iter().zip(type_vars2.iter()) {
                    constrain_because(ctx, tv1, tv2, constraint_reason.clone());
                }
            }
            _ => {}
        }
...
fn generate_constraints_expr(
    ...
    match &*expr.kind {
        ...
        ExprKind::MemberFuncAp(receiver_expr, fname, args) => {
            ...
            } else if let Some((enum_def, variant)) = infer_enum(&mode, &fname.v) {
...
fn generate_constraints_pat(ctx: &mut StaticsContext, mode: Mode, pat: &Rc<Pat>) {
    ...
    match &*pat.kind {
        ...
        PatKind::Variant(prefixes, tag, data) => {
            ...
            } else if let Some((enum_def, idx)) = infer_enum(&mode, &tag.v) {
                ctx.resolution_map.insert(
                    tag.id,
                    Declaration::EnumVariant {
                        e: enum_def.clone(),

...
fn get_files(
    ...
    for root in roots {
        let main_file_data = file_provider.search_for_file(Path::new(root)).unwrap();
    ...
    }
    {
        let prelude_file_data =
            FileData::new("prelude.en".into(), "prelude.en".into(), PRELUDE.into());
...
fn add_imports(
    ...
    for item in file_ast.items.iter() {
        if let ItemKind::Import(ident, _) = &*item.kind {
            let path = PathBuf::from(format!("{}.en", ident.v));
...
impl MockFileProvider {
    pub fn new(mut path_to_file: HashMap<PathBuf, String>) -> Box<Self> {
        path_to_file.insert(Path::new("prelude.en").to_path_buf(), PRELUDE.into());
        Box::new(Self {
            path_to_file,
    ...
    pub fn single_file(contents: &str) -> Box<Self> {
        ...
        path_to_file.insert(Path::new("prelude.en").to_path_buf(), PRELUDE.into());
        path_to_file.insert(Path::new("main.en").to_path_buf(), contents.into());
        Box::new(Self {
            path_to_file,
            _shared_objects_dir: PathBuf::new(),
        })
